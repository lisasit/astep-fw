...
class Decode:
    ...
    def prepare_readout(self, readout: bytearray) -> list:
        list_hits = []
        b = 0
        packet_len = 0
        while b < len(readout):
    ...
    def decode_readout(self, logger, readout: bytearray, i: int, sample_clock_period_ns: int = 10, printer: bool = True) -> pd.DataFrame:
        ...
        list_hits, packet_len = self.prepare_readout(readout)
        ...
        for hit in list_hits:
            ...
            try:
                ...
                fpga_ts     = int.from_bytes(hit[7:11], 'big')
    ...
    def decode_readout_v4(self, logger, readout: bytearray, i: int, sample_clock_period_ns: int = 25, printer: bool = True, use_negedge_ts: bool = True) -> pd.DataFrame:
        """
        ...
        for hit in list_hits:
            try:
                ...
                ts1_dec = self.gray_to_dec((ts1 << 3) + tsfine1) << 1 | (tsneg1 & use_negedge_ts)
                ts2_dec = self.gray_to_dec((ts2 << 3) + tsfine2) << 1 | (tsneg2 & use_negedge_ts)
